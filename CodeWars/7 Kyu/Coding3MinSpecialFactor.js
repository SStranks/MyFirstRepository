...
function sc(n) {
  ...
  let nFactors = [];
  for (let i = 0; i <= n; i++){
    if (n % i === 0) nFactors.push(i.toString(2))
  }
  return nFactors.filter((el) => RegExp(`(${el})`).test(nBinary)).map((el) => parseInt(el, 2))
}

// Other Solutions
function sc(n){
  let compare = n.toString(2);
  let response = [];
  
  for(let i = 1; i <= n; i++){
    if(compare.includes(i.toString(2)) && (n % i === 0)){
      response.push(i)
    }
  }
  return response;
