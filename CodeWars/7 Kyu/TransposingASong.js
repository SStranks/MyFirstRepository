...
function transpose(song, interval) {
  ...
  return song.map((el) => {
    if (el.length === 1 || el[1] === '#') return sharp[(12 + sharp.indexOf(el) + interval) % 12];
    return sharp[(12 + flat.indexOf(el) + interval) % 12];
  });
}
...
function transpose(song, interval) {
  ...
  return song.map(
    (note) => sharp[(Math.max(flat.indexOf(note), sharp.indexOf(note)) + interval + 12) % 12]
  );
}
